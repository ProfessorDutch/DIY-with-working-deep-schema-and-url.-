...
export async function migrateContent(msaKey: string, sourceRow: ContentRow) {
  try {
    ...
    // Generate a unique URL slug by combining the MSA city, case type, and a unique identifier
    const uniqueSlug = `${msaData.city.toLowerCase()}-${sourceRow.case_type.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`;

    // Create new content row
    const updatedContent = {
      url_slug: uniqueSlug, // Use the unique slug to ensure no duplicates
      city: msaData.city,
      state: msaData.state,
      ...
      schema_markup: updatedSchema // Directly use the object instead of stringifying
    };
